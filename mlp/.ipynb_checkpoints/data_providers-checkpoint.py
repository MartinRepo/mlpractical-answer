...
class MNISTDataProvider(DataProvider):
    ...
    def to_one_of_k(self, int_targets):
        ...
        one_of_k = np.zeros((num_data, self.num_classes))
        one_of_k[np.arange(num_data), int_targets] = 1
        return one_of_k
...
class MetOfficeDataProvider(DataProvider):
    ...
    def __init__(self, window_size, batch_size=10, max_num_batches=-1,
        ...
        raw_data = np.loadtxt(data_path, usecols=range(2, 33), skiprows=3)
        ...
        num_data_points = len(normalized_data) - window_size + 1
        ...
        # targets = ...
        inputs = np.array([normalized_data[i:i + window_size - 1] for i in range(num_data_points)])
        targets = np.array([normalized_data[i + window_size - 1] for i in range(num_data_points)])

        # if shuffle_order:
        # if max_num_batches != -1:
